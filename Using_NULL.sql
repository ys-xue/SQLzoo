...
select dept.name, count( distinct teacher.name) from teacher right join dept
on teacher.dept=dept.id
group by dept.name
...
select teacher.name,
CASE when dept is NULL then 'Art' ELSE 'Sci' end as type
from teacher
...
select teacher.name, 
case when dept=3 then 'Art'  when dept is null then  'None' else 'Sci' end as type
from teacher
